    ...
    module.exports = function (seriesType, ecModel, api, payload) {
        ecModel.eachSeriesByType(seriesType, function (seriesModel) {
            ...
            if(seriesModel.option.label.normal){
                ...
                // seriesModel.option.label.normal.color = seriesModel.option.label.color;
            ...
            // echarts4的情况label的布局及样式只能在normal中生效，并且需要加上|{b}后缀才能应用rich样式
            // if(seriesModel.option.label.formatter && seriesModel.option.label.formatter.indexOf("|{") < 0){
            //     seriesModel.option.label.normal.formatter = seriesModel.option.label.formatter;
            //     seriesModel.option.label.normal.rich = seriesModel.option.label.rich;
            //     seriesModel.option.label.normal.formatter = seriesModel.option.label.normal.formatter.replace("b", "b|{b}");
            //     seriesModel.option.label.normal.formatter = seriesModel.option.label.normal.formatter.replace("c", "c|{c}");
            //     seriesModel.option.label.normal.formatter = seriesModel.option.label.normal.formatter.replace("d", "d|{d}");
            //     seriesModel.option.label.normal.formatter = seriesModel.option.label.normal.formatter.replace("{b|{b}} :", "{b|{b} :}");
            //     // seriesModel.option.label.normal.formatter = seriesModel.option.label.normal.formatter.replace("{c|{c} \n", "{c|{c \n}}");
            //     seriesModel.option.label.normal.formatter = seriesModel.option.label.normal.formatter.replace("{d|{d}}%", "{d|{d}%}");
            //     seriesModel.option.label.formatter = seriesModel.option.label.normal.formatter;
            //     // delete seriesModel.option.label.normal.formatter;
            // }
            ...
            data.each('value', function (value, idx) {
                ...
                // 玫瑰图的情况重新计算扇形最大半径
                if(roseType){
                    var rRose = numberUtil.linearMap(value, extent, [r0, r]);
                    // 椭圆横轴半径
                    a = rRose*131/150;
                    // 椭圆纵轴半径
