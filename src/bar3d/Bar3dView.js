...
function updateCube(el, layout, isHorizontal, animationModel, newIndex) {
    ...
    el.eachChild(function (childEl) {
        ...
        } else if (childEl.name == 'leftFlank') {
            graphic.updateProps(childEl, { shape: getFlankShape(shape, points, isHorizontal) }, animationModel, newIndex);
        } else if(childEl.name == 'rightFlank'){
            graphic.updateProps(childEl, { shape: getRightFlankShape(shape, points, isHorizontal) }, animationModel, newIndex);
        } else {
...
function getPoints(shape, isHorizontal) {
    ...
    var r = length;
    var cosY = r * Math.cos(angle) / 2;
    var sinX = r * Math.sin(angle) / 2;
 ...
 * @param {*} opacity 
 */
function setStyle(path, barStyle, fillColor, opacity) {
    path && path.useStyle(zrUtil.defaults(
        {
            fill: path.fillNone ? 'none' : fillColor,
            opacity: opacity,
            lineJoin: 'round'
        },
        barStyle
...
function updateCubeStyle(el, data, dataIndex, itemModel, layout, seriesModel, isHorizontal) {
    ...
    // 设置顶部样式
    setStyle(top, barStyle, liftColor(topColor, -0.2), opacity);

    // 不显示的面不进行fill只需stroke
    ...
    // 设置底部样式
    setStyle(bottom, backStyle, liftColor(topColor, -0.2), opacity);

    //设置右侧面样式
    setStyle(rightFlank, backStyle, liftColor(topColor, -0.2), opacity);

    // 设置左侧面样式
    setStyle(leftFlank, barStyle, lerpColor(color, 0.2), opacity);

    //设置正面样式
    setStyle(front, barStyle, color, opacity);
    ...
    // 设置鼠标移上样式
    helper.setHoverStyle(el, hoverStyle, topColor);
}
...
function getCylinderTop(shape, isHorizontal) {
    ...
    return isHorizontal ? {
        ...
        rx: shape.width / 2 - 1, // 减1，因为看椭圆看起来比正方体大
        ry: shape.width / 8
    } : {
...
function createCylinder(data, dataIndex, itemModel, layout, isHorizontal, animationModel, isUpdate) {
    ...
    // 绘制正面
    var front = new graph.QuadrangleArc({
        name: 'front',
