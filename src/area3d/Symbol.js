...
symbolProto.link = function (data, idx, symbolType, symbolSize, seriesScope) {
    ...
    var yValue = data.getRawDataItem(idx);
...
symbolProto._updateCommon = function (data, idx, symbolSize, seriesScope) {
    ...
    var x = parsePercent(symbolOffset[0], symbolSize[0]), 
        y = Math.abs(parsePercent(symbolOffset[1], symbolSize[1]));

    if (symbolOffset) {// 这个偏移进行修改与echarts设置不一样了
        var yValue = data.getRawDataItem(idx);
        symbolPath.attr('position', [x, yValue >= 0 ? -y : y]);
    }
